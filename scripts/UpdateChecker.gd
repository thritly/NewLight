...
func get_current_version() -> String:
	# 优先尝试读取项目资源中的 config.json
	var paths = ["res://config.json", "user://version.txt"]
	...
	for path in paths:
		if FileAccess.file_exists(path):
			...
			if json_data is Dictionary and json_data.has("version"):
				# 如果是 JSON 且包含 version 字段，返回其值
				return str(json_data["version"])
			else:
				# 兼容旧版本：如果内容不是 JSON 或没有 version 字段，
				# 直接返回原内容（可能是纯文本版本号）
				return content
